...
import { runClientsAdd, runClientsArchive, runClientsDelete, runClientsGet, runClientsList, runClientsPromote, runClientsUpdate, } from "./cli/commands/clients.js";
import { runTimeCancel, runTimeDelete, runTimeGet, runTimeList, runTimeLog, runTimeStart, runTimeStatus, runTimeStop, runTimeSummary, runTimeUpdate, runTimerUpdate, } from "./cli/commands/time.js";
import { runInvoicesCreate, runInvoicesDelete, runInvoicesGet, runInvoicesList, runInvoicesOverdue, runInvoicesPay, runInvoicesPdf, runInvoicesRender, runInvoicesStatus, runPaymentsDelete, runPaymentsList, } from "./cli/commands/invoices.js";
import { runCrmFollowups, runCrmNote, runCrmNoteDelete, runCrmNoteGet, runCrmNoteUpdate, runCrmNotes, } from "./cli/commands/crm.js";
import { runExpensesAdd, runExpensesDelete, runExpensesGet, runExpensesList, runExpensesUpdate, } from "./cli/commands/expenses.js";
import { runRecurringAdd, runRecurringDelete, runRecurringGet, runRecurringList, runRecurringRun, runRecurringUpcoming, runRecurringUpdate, } from "./cli/commands/recurring.js";
...
import { runTemplatePath, runTemplatePrint, runTemplateReset, runTemplateShowDefault, runTemplateWrite, } from "./cli/commands/template.js";
...
program
    .command("install")
    .alias("setup")
    .description("Wire the fretwork MCP server + skill into your AI agent (Claude Code, Claude Desktop, Cursor, OpenCode, Codex, Hermes, OpenClaw, Grok Build)")
    ...
    .option("--host <ids>", "Wire these hosts without prompting (comma-separated: claude, claude-desktop, cursor, opencode, codex, hermes, openclaw, grok). Repeatable.", (v, prev = []) => [...prev, v])
...
program
    ...
    process.stderr.write(`✓ fretwork ${before} → ${after}. Your data in ~/.fretwork is untouched. ` +
...
invoices
    ...
    .option("--notes <text>", "Free-form notes")
    .option("--item <desc|qty|rate[|unit]>", "Add an explicit line item; repeat as needed", (val, prev = []) => [...prev, val])
    .option("--json", "Emit JSON")
...
recurring
    ...
    .option("--item <desc|qty|rate[|unit[|kind]]>", "Add a line item; repeat as needed", (val, prev = []) => [...prev, val])
...
config
    ...
    .option("--custom-instructions <text>", "Free-form invoice design notes (rendered as a comment in the output)")
    .option("--rate <rate>", "Default hourly rate (default 100)")
    .option("--tax-rate <pct>", "Tax rate as percent")
    .option("--currency <code>", "Currency")
    .option("--due-days <days>", "Payment terms: days until an invoice is due, e.g. 14 or 30 (default 14)")
