...
export function userTemplateDir() {
    return join(fretworkHome(), "templates");
}
export function userTemplatePath() {
    return join(userTemplateDir(), USER_TEMPLATE_NAME);
}
export function userCssPath() {
...
export function bundledTemplatePath() {
    const here = fileURLToPath(new URL(".", import.meta.url));
    return pathResolve(here, "../../../templates", USER_TEMPLATE_NAME);
}
export function readBundledTemplate() {
...
export function readUserTemplate() {
...
export function writeUserTemplate(html) {
...
function readTemplateWithFallback() {
    const userPath = userTemplatePath();
    if (!existsSync(userPath)) {
...
function logoHtml(businessLogo, businessName) {
    if (!businessLogo)
...
// Line-item helpers
// ─────────────────────────────────────────────────────────────────────────
function inferKind(unitLabel, fallback) {
    if (fallback)
        return fallback;
    const u = (unitLabel ?? "").toLowerCase().trim();
    if (!u)
        return "flat";
    if (/^(hr|hour|hours)$/.test(u))
        return "hours";
    if (/^(week|weeks|wk)$/.test(u))
        return "weekly";
    if (/^(month|months|mo|monthly)$/.test(u))
        return "subscription";
    if (/(milestone|deliverable|fixed|flat)/.test(u))
        return "flat";
    if (/(reimb|expense|expenses|card)/.test(u))
        return "expense";
    return "flat";
}
const DOT_COLOR = {
...
function subDescription(kind, unit, rate, currency) {
    switch (kind) {
        case "hours":
...
function formatQty(quantity, unit, kind) {
    const u = unit ?? defaultUnitFor(kind);
    if (kind === "hours")
        return `${quantity.toFixed(2)} ${u}`;
...
function softFromAccent(accent) {
...
export function renderInvoiceHtml(numberOrId, _opts = {}) {
    const inv = requireInvoice(numberOrId);
    const client = requireClient(inv.clientSlug);
    const cfg = getConfig();
    ...
    const hasMeaningfulQty = inv.lineItems.some((li) => {
        ...
        if (kind === "subscription" || kind === "flat")
    ...
    if (fallbackReason) {
        ...
        const warning = `Your invoice template at ${userTemplatePath()} did not parse and was ` +
...
function splitLines(address, city) {
    const out = [];
    if (address) {
