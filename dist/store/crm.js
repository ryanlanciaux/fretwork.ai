...
export function addCrmNote(input) {
...
export function listCrmNotes(filter = {}) {
    const db = openDb();
    const conds = [];
    if (filter.client) {
...
export function getCrmNote(id) {
    const db = openDb();
    const row = db.select().from(crmTable).where(eq(crmTable.id, id)).all()[0];
    return row ? rowToNote(row) : null;
}
export function requireCrmNote(id) {
    const n = getCrmNote(id);
    if (!n)
        throw new Error(`CRM note not found: ${id}`);
    return n;
}
export function updateCrmNote(id, patch) {
...
export function deleteCrmNote(id) {
    const cur = requireCrmNote(id);
    const db = openDb();
...
export function listFollowups(opts = {}) {
    ...
    for (const c of active) {
        ...
        const explicitDue = notes.find((n) => n.followupAt && (!opts.dueBy || n.followupAt <= opts.dueBy) && n.followupAt <= today);
    ...
    return followups.sort((a, b) => {
        if (a.daysSinceContact === null && b.daysSinceContact === null)
            return 0;
        if (a.daysSinceContact === null)
            return -1;
        if (b.daysSinceContact === null)
            return 1;
