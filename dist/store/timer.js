...
export function startTimer(input = {}) {
    ...
    if (existing) {
        ...
        throw new Error(`A timer is already running for ${target} (started ${minutes}m ago). ` +
...
export function updateActiveTimer(patch) {
    const current = getActiveTimer();
    if (!current)
        throw new Error("No active timer.");
    const clientSlug = patch.client === undefined
        ? current.clientSlug
...
export function stopTimer(input = {}) {
    ...
    if (!clientArg) {
        throw new Error("Timer has no client. Pass --client to associate it with one (or cancel the timer).");
    }
