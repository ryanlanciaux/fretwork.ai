...
export function isImageDataUri(v) {
...
export function allowedLogoPath(p) {
    let real;
    try {
    ...
    if (!real.startsWith(logoRoot() + sep))
        return null;
    if (!LOGO_MIME_BY_EXT[extname(real).toLowerCase()])
        return null;
    return real;
}
export function logoGuidance() {
    return (`Logos must be an image file (${Object.keys(LOGO_MIME_BY_EXT).join(", ")}) inside ${logoRoot()} — ` +
 ...
 */
export function validateLogoValue(v) {
    if (isImageDataUri(v))
        return v;
    if (isUrl(v))
        throw new Error(`businessLogo "${v.slice(0, 60)}" is a URL. ${logoGuidance()}`);
    const real = allowedLogoPath(v);
    if (!real)
        throw new Error(`businessLogo "${v}" is not an image file inside ${logoRoot()}. ${logoGuidance()}`);
    return real;
}
/**
 ...
 */
export function stageLogoFile(input) {
    if (isImageDataUri(input))
        return input;
    if (isUrl(input)) {
        throw new Error(`"${input.slice(0, 60)}" is a URL — logo URLs aren't supported (invoices render offline). ` +
