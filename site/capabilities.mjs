// Single source of truth for the capabilities table (site + CAPABILITIES.md).
// `npm run site` verifies every MCP tool in src/mcp.ts appears here exactly
// once, so the table can't drift from the code.
export const AREAS = [
  { name: "Clients", rows: [
...
// CLI-only conveniences (no MCP equivalent needed).
