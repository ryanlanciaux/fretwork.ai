...
function isCadence(s: string): s is RecurringCadence {
  return (CADENCES as string[]).includes(s);
}

function parseTemplate(raw: string): RecurringInvoiceTemplate {
  let parsed: unknown;
  try {
  ...
  return parsed as RecurringInvoiceTemplate;
}

function rowToRecurring(row: typeof recurringTable.$inferSelect): RecurringInvoice {
  return {
    id: row.id,
...
export function addRecurringInvoice(input: AddRecurringInvoiceInput): RecurringInvoice {
...
export function getRecurringInvoice(id: string): RecurringInvoice | null {
  const db = openDb();
  const row = db.select().from(recurringTable).where(eq(recurringTable.id, id)).all()[0];
  return row ? rowToRecurring(row) : null;
}

export function requireRecurringInvoice(id: string): RecurringInvoice {
  const r = getRecurringInvoice(id);
  if (!r) throw new Error(`Recurring invoice not found: ${id}`);
  return r;
}

export function listRecurringInvoices(filter: ListRecurringFilter = {}): RecurringInvoice[] {
  const db = openDb();
  const conds = [];
  if (filter.client) {
...
export function updateRecurringInvoice(
  id: string,
  patch: UpdateRecurringInvoiceInput,
): RecurringInvoice {
...
export function deleteRecurringInvoice(id: string): RecurringInvoice {
...
export function runRecurringInvoices(opts: { asOf?: string } = {}): RecurringRunResult {
  ...
  const dueRows = db
    ...
    .from(recurringTable)
    .where(
      and(eq(recurringTable.active, true), lte(recurringTable.nextIssueAt, asOf)),
    )
    .all();

  const generated: RecurringRunResult["generated"] = [];
  const skipped: RecurringRunResult["skipped"] = [];
  ...
  for (const row of dueRows) {
    ...
    try {
      const tx = sqlite.transaction(() => {
        ...
        if (r.autoSend) setInvoiceStatus(inv.number, "sent");
...
export function upcomingRecurringInvoices(
  opts: { count?: number; client?: string } = {},
): UpcomingRecurringRow[] {
