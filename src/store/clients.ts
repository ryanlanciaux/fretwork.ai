...
import {
  activeTimer as activeTimerTable,
  clients as clientsTable,
  crmNotes as crmNotesTable,
  expenses as expensesTable,
  invoices as invoicesTable,
  recurringInvoices as recurringTable,
...
function rowToClient(row: typeof clientsTable.$inferSelect): Client {
  return {
    id: row.id,
    slug: row.slug,
    name: row.name,
...
export function listClients(opts: { status?: ClientStatus | ClientStatus[] } = {}): Client[] {
  const db = openDb();
  const rows = (() => {
    if (!opts.status) return db.select().from(clientsTable).orderBy(asc(clientsTable.name)).all();
...
export function updateClient(slugOrId: string, patch: UpdateClientInput): Client {
  ...
  db.update(clientsTable)
    .set({
      ...
      defaultRate:
        patch.defaultRate === undefined ? current.defaultRate : patch.defaultRate,
...
const PROMOTION_ORDER: ClientStatus[] = ["prospect", "lead", "client"];

export function promoteClient(slugOrId: string, to?: ClientStatus): Client {
  const current = requireClient(slugOrId);
  const target: ClientStatus = (() => {
    if (to) return to;
...
export function archiveClient(slugOrId: string): Client {
  return promoteClient(slugOrId, "archived");
}

export interface ClientReferenceCounts {
  timeEntries: number;
  expenses: number;
  invoices: number;
  crmNotes: number;
...
function countClientReferences(slug: string): ClientReferenceCounts {
  const db = openDb();
  return {
    timeEntries: db
...
export function deleteClient(slugOrId: string): Client {
  const current = requireClient(slugOrId);
  ...
  if (total > 0) {
    const parts: string[] = [];
    if (refs.timeEntries) parts.push(`${refs.timeEntries} time entries`);
    if (refs.expenses) parts.push(`${refs.expenses} expenses`);
    if (refs.invoices) parts.push(`${refs.invoices} invoices`);
    if (refs.crmNotes) parts.push(`${refs.crmNotes} CRM notes`);
    if (refs.recurringInvoices) parts.push(`${refs.recurringInvoices} recurring invoices`);
    if (refs.activeTimer) parts.push(`an active timer`);
    throw new Error(
      `Cannot delete ${current.slug}: still referenced by ${parts.join(", ")}. ` +
