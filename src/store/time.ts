...
function rowToTime(row: typeof timeTable.$inferSelect): TimeEntry {
  return {
    id: row.id,
...
export function listTimeEntries(filter: ListTimeFilter = {}): TimeEntry[] {
  const db = openDb();
  const conditions = [];
  if (filter.client) {
...
export function summariseTime(filter: ListTimeFilter = {}): TimeSummaryRow[] {
  const entries = listTimeEntries(filter);
  const cfg = getConfig();
  const map = new Map<string, TimeSummaryRow>();
  for (const e of entries) {
    ...
    if (!e.billed) cur.unbilledHours = round2(cur.unbilledHours + e.hours);
    cur.entries += 1;
...
export function requireTimeEntry(id: string): TimeEntry {
  const e = getTimeEntry(id);
  if (!e) throw new Error(`Time entry not found: ${id}`);
  return e;
}
...
export function updateTimeEntry(id: string, patch: UpdateTimeEntryInput): TimeEntry {
  const cur = requireTimeEntry(id);
  if (cur.billed) {
    throw new Error(
      `Time entry ${id} is on invoice ${cur.invoiceNumber}. Delete that invoice ` +
...
export function deleteTimeEntry(id: string): TimeEntry {
  ...
  if (cur.billed) {
    throw new Error(
      `Time entry ${id} is on invoice ${cur.invoiceNumber}. Delete the invoice ` +
...
export function markEntriesBilled(entryIds: string[], invoiceNumber: string): void {
...
export function unmarkEntriesByInvoice(invoiceNumber: string): void {
  const db = openDb();
  db.update(timeTable)
