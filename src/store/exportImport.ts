...
export interface ImportSummary {
  ...
  errors: string[];
  /** Snapshot written before a `replace` wiped the tables (null for merge). */
...
const TABLE_NAMES_SQL: Record<string, string> = {
  clients: "clients",
  config: "config",
...
export function importSnapshot(
  ...
  if (snapshot.version !== SNAPSHOT_VERSION) {
    throw new Error(
      `Snapshot version ${snapshot.version} cannot be imported by this version ` +
    ...
    for (const { name, table, pk } of ORDER) {
      ...
      for (const row of rows) {
        try {
          ...
          db.insert(table as any).values(row as any).run();
          ins += 1;
        } catch (e) {
          errors.push(
...
export function snapshotToCsvFiles(snapshot: Snapshot): Record<string, string> {
  const files: Record<string, string> = {};
  for (const { name } of ORDER) {
    const rows = (snapshot as unknown as Record<string, unknown[]>)[name] ?? [];
    files[`${TABLE_NAMES_SQL[name]}.csv`] = rowsToCsv(rows as Record<string, unknown>[]);
  }
  return files;
}
