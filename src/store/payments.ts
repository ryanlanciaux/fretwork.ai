...
import type { InvoiceStatus, Payment, RecordPaymentInput } from "./types.js";
...
function rowToPayment(row: typeof paymentsTable.$inferSelect): Payment {
  return {
    id: row.id,
...
export function paymentTotalsByInvoiceId(): Map<string, number> {
  const db = openDb();
  const rows = db
    .select({
  ...
  for (const r of rows) out.set(r.invoiceId, round2(r.total));
  return out;
}

export function paymentsTotalFor(invoiceId: string): number {
  const db = openDb();
  const row = db
...
export function listPayments(invoiceNumberOrId?: string): Payment[] {
  const db = openDb();
  ...
  if (!inv) throw new Error(`Invoice not found: ${invoiceNumberOrId}`);
  return db
    .select()
    .from(paymentsTable)
...
export function requirePayment(id: string): Payment {
...
export function recordPayment(input: RecordPaymentInput): RecordPaymentResult {
  ...
  if (!inv) throw new Error(`Invoice not found: ${input.invoice}`);
  if (inv.status === "void") {
  ...
  return {
    payment,
    invoiceNumber: inv.number,
    status: fullyPaid ? "paid" : (inv.status as InvoiceStatus),
    total: inv.total,
    amountPaid,
...
export function deletePayment(id: string): RecordPaymentResult {
  const payment = requirePayment(id);
  const inv = findInvoiceRowById(payment.invoiceId);
  if (!inv) throw new Error(`Invoice not found for payment ${id}`);
  ...
  return {
    payment,
    invoiceNumber: inv.number,
    status: reopened ? "sent" : (inv.status as InvoiceStatus),
    total: inv.total,
    amountPaid,
    balanceDue: round2(Math.max(0, inv.total - amountPaid)),
...
// ── local invoice-row lookups (kept here to avoid importing ./invoices) ──

type InvoiceRow = typeof invoicesTable.$inferSelect;

function findInvoiceRow(numberOrId: string): InvoiceRow | null {
  const db = openDb();
  const byNumber = db
    .select()
    .from(invoicesTable)
  ...
  return (
    db.select().from(invoicesTable).where(eq(invoicesTable.id, numberOrId)).all()[0] ?? null
  );
}

function findInvoiceRowById(id: string): InvoiceRow | null {
  const db = openDb();
