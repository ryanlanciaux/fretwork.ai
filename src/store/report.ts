...
import type { FinancialReport, InvoiceStatus } from "./types.js";
import { round2 } from "./util.js";

export function financialReport(opts: { from?: string; to?: string } = {}): FinancialReport {
  const all = listInvoices();
  const within = all.filter((i) => {
    if (opts.from && i.issuedAt < opts.from) return false;
    if (opts.to && i.issuedAt > opts.to) return false;
    return true;
  });
  ...
  const byClient = new Map<string, { revenue: number; outstanding: number }>();
  let outstanding = 0;
  let collected = 0;
  for (const inv of within) {
    ...
    if (inv.status === "paid") cur.revenue = round2(cur.revenue + inv.total);
