...
import type {
  ...
  InvoiceStatus,
  OverdueInvoice,
  ReconcileOverdueResult,
  UpdateInvoiceInput,
} from "./types.js";

function rowToInvoice(row: typeof invoicesTable.$inferSelect, amountPaid = 0): Invoice {
...
function rowToLineItem(row: typeof lineItemsTable.$inferSelect): InvoiceLineItem {
  return {
    id: row.id,
...
function normalizeLineItemKind(raw: string | null): InvoiceLineItem["kind"] {
...
export function generateInvoiceNumber(year = new Date().getUTCFullYear()): string {
  const db = openDb();
...
export function listInvoices(
  opts: { client?: string; status?: InvoiceStatus | InvoiceStatus[] } = {},
): Invoice[] {
  const db = openDb();
  const conds = [];
  if (opts.client) {
...
export function getInvoice(numberOrId: string): InvoiceDetail | null {
  const db = openDb();
...
export function requireInvoice(numberOrId: string): InvoiceDetail {
  const inv = getInvoice(numberOrId);
  if (!inv) throw new Error(`Invoice not found: ${numberOrId}`);
  return inv;
}

export function createInvoice(input: CreateInvoiceInput): InvoiceDetail {
  const cfg = getConfig();
  const client = requireClient(input.client);

  const issuedAt = input.issuedAt ?? todayIso();
  if (!isIsoDate(issuedAt)) throw new Error(`issuedAt must be YYYY-MM-DD`);
  const dueAt = input.dueAt ?? addDaysIso(issuedAt, cfg.dueDays);
  if (!isIsoDate(dueAt)) throw new Error(`dueAt must be YYYY-MM-DD`);
  ...
  if (items.length === 0) {
    throw new Error(
      "Cannot create an invoice with no line items. Pass fromTimeRange or lineItems.",
    );
  }
  ...
  const tx = sqlite.transaction(() => {
    ...
    if (billedEntryIds.length > 0) markEntriesBilled(billedEntryIds, number);
  });
  tx();
...
export function updateInvoice(numberOrId: string, patch: UpdateInvoiceInput): InvoiceDetail {
...
export function setInvoiceStatus(numberOrId: string, status: InvoiceStatus): InvoiceDetail {
  const cur = requireInvoice(numberOrId);
  const db = openDb();
  const now = Date.now();
...
export function listOverdueInvoices(opts: { asOf?: string } = {}): OverdueInvoice[] {
  const asOf = opts.asOf ?? todayIso();
  if (!isIsoDate(asOf)) throw new Error(`asOf must be YYYY-MM-DD (got: ${asOf})`);
  const candidates = listInvoices({ status: ["sent", "overdue"] });
  const out: OverdueInvoice[] = [];
  for (const inv of candidates) {
    if (inv.paidAt) continue;
    if (inv.dueAt >= asOf) continue;
...
export function reconcileOverdueInvoices(opts: { asOf?: string } = {}): ReconcileOverdueResult {
  const asOf = opts.asOf ?? todayIso();
  const due = listOverdueInvoices({ asOf });
  const updated: Invoice[] = [];
  const alreadyOverdue: Invoice[] = [];
...
export function deleteInvoice(numberOrId: string): void {
  const cur = requireInvoice(numberOrId);
  const sqlite = rawSqlite();
  const tx = sqlite.transaction(() => {
