...
export function isHostId(v: string): v is HostId {
  return (ALL_HOST_IDS as readonly string[]).includes(v);
}
...
export function detectHosts(): HostDetections {
  const home = homedir();
  ...
  return {
    ...
    claudeDesktop:
      existsSync(claudeDesktopCfg) || existsSync(dirname(claudeDesktopCfg)),
...
export function wireClaude(ctx: WireContext): WireResult {
  if (!cmdExists("claude")) {
    return {
      ok: false,
      message:
...
export function wireClaudeDesktop(ctx: WireContext): WireResult {
...
export function wireCursor(ctx: WireContext): WireResult {
...
export function wireOpenCode(ctx: WireContext): WireResult {
...
export function codexHint(ctx: WireContext): string {
  return (
...
export function wireHermes(ctx: WireContext): WireResult {
  const hint =
    `hermes CLI not found. Once Hermes is installed, run:\n` +
    `  hermes mcp add ${ctx.binName} --command ${ctx.mcpCmd}`;
  if (!cmdExists("hermes")) return { ok: false, message: hint };
...
export function wireOpenClaw(ctx: WireContext): WireResult {
  const cfgPath = resolve(openclawStateDir(), "openclaw.json");
  const snippet =
    `  "mcp": { "servers": { "${ctx.binName}": { "command": "${ctx.mcpCmd}" } } }`;
  ...
  const merged = mergeJsonFile(cfgPath, (cfg) => {
    const mcp = (cfg.mcp ??= {} as Record<string, unknown>) as Record<string, unknown>;
...
export function wireGrok(ctx: WireContext): WireResult {
  const toml =
    `Add to ${resolve(grokHome(), "config.toml")}:\n` +
    `  [mcp_servers.${ctx.binName}]\n  command = "${ctx.mcpCmd}"`;
  if (!cmdExists("grok")) return { ok: false, message: `grok CLI not found. ${toml}` };
...
export function skillDirFor(host: HostId, projectDir?: string): string | null {
  const home = homedir();
  switch (host) {
    case "claude":
      return projectDir
...
export function manualConfigsSnippet(ctx: WireContext): string {
  return [
...
export function installSkill(targetDir: string, skillSrc: string): WireResult {
  try {
 ...
 */
export function resolveMcpServerPath(): string | null {
