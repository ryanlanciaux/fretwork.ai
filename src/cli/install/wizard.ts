...
import { readInstallMeta, writeInstallMeta, type InstallMeta } from "./meta.js";

export interface InstallWizardArgs {
  /** Print manual configs and exit 0 — used by CI / curl|sh-no-tty. */
  nonInteractive?: boolean;
  /** When invoked from install.sh, the parent already showed a banner. */
  skipBanner?: boolean;
  /** Override binary name registered with hosts (default: "fretwork"). */
  name?: string;
  /**
   * Hosts to wire without prompting (comma-separated ids, or repeated).
   * Used by AI agents following the /agent/<token> runbook:
   *   fretwork install --host hermes --yes
   */
  hosts?: string[];
  /** Skip every prompt; implies `hosts` (or auto-detected hosts when empty). */
  yes?: boolean;
}
...
function safeStageLogo(input: string): string | null {
  try {
...
function hasBootstrappedBusiness(): boolean {
  const cfg = getConfig();
  return (
    cfg.businessName !== DEFAULT_BUSINESS_NAME ||
...
function validateRequired(v: string): string | undefined {
...
function validateEmail(v: string): string | undefined {
  ...
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(t)) return "Doesn't look like an email.";
  return undefined;
}

function validateNumber(v: string): string | undefined {
  if (!v.trim()) return undefined; // empty = keep current
  if (!Number.isFinite(Number(v))) return "Expected a number.";
  return undefined;
}
...
async function promptBusinessProfile(): Promise<void> {
  ...
  if (isCancel(proceed) || proceed === false) {
    if (isFirstRun) {
      log.warn(
        "Skipping — invoices will show placeholder business info until you run `fretwork init` or `fretwork config set`.",
      );
    }
    return;
  }
  ...
  if (tagline === undefined) return savePartial({ name, email });
  ...
  if (address === undefined) return savePartial({ name, email, tagline });
  ...
  if (city === undefined) return savePartial({ name, email, tagline, address });
  ...
  if (phone === undefined) return savePartial({ name, email, tagline, address, city });
...
function savePartial(p: PartialProfile): void {
  saveProfile(p);
  log.warn("Stopped partway through — saved what you'd entered. Re-run any time.");
}

function saveProfile(p: PartialProfile): void {
  const current = getConfig();
  const saved = bootstrapConfig({
    businessName: p.name,
    businessEmail: p.email,
    businessTagline:
      p.tagline === undefined ? current.businessTagline : p.tagline || null,
    businessAddress:
      p.address === undefined ? current.businessAddress : p.address || null,
  ...
  log.success(
    `Saved business profile (${pc.bold(saved.businessName)}, ${saved.currency}, ${saved.dueDays}-day terms).`,
  );
}

// One host → one wire call. Shared by the interactive wizard and the
// unattended `--host … --yes` path so both behave identically.
function wireHost(host: HostId, ctx: WireContext): WireResult | null {
  switch (host) {
    case "claude":
...
function parseHostList(raw: string[] | undefined): { hosts: HostId[]; bad: string[] } {
  const hosts: HostId[] = [];
  const bad: string[] = [];
...
async function runUnattended(args: InstallWizardArgs, binName: string, mcpCmd: string): Promise<number> {
  const { hosts: requested, bad } = parseHostList(args.hosts);
  if (bad.length) {
    process.stderr.write(
      pc.red(`Unknown host id(s): ${bad.join(", ")}. Valid: claude, claude-desktop, cursor, opencode, codex, hermes, openclaw, grok, manual\n`),
    );
    return 1;
  }
  let hosts = requested;
  ...
  const ctx: WireContext = { binName, mcpCmd, scope: "user" };
  const wired: string[] = [];
  let failures = 0;
  ...
  if (!skillSrc) process.stdout.write(pc.yellow("! Could not locate SKILL.md — skipped skill installs.\n"));
...
export async function runInstallWizard(args: InstallWizardArgs = {}): Promise<number> {
  ...
  if (!mcpCmd) {
    process.stderr.write(
      pc.red(
        `Could not locate \`fretwork-mcp-server\` on this machine.\n` +
          `Either:\n` +
  ...
  if (args.nonInteractive) {
    process.stdout.write(
      pc.dim(
        "Non-interactive install — skipping wizard.\n" +
          "Run `fretwork install` later to wire up MCP hosts.\n\n",
      ),
    );
    process.stdout.write(
      manualConfigsSnippet({ binName, mcpCmd, scope: "user" }) + "\n",
    );
    return 0;
  }
  ...
  if (scope === "project") {
    ...
    log.warn(
      `Cursor / OpenCode / Codex configs are profile-wide — only Claude Code can be wired for a single project here.`,
    );
  }

  // 2. Host multiselect.
  const hosts = detectHosts();
  const options: Array<{ value: HostId; label: string; hint: string }> = [
    {
      value: "claude",
      label: "Claude Code",
  ...
  const ctx: WireContext = { binName, mcpCmd, scope, projectDir };

  // 3. Apply each selection.
  const wiredHosts: string[] = [];
  if (selected.size === 0 || (selected.size === 1 && selected.has("manual"))) {
    ...
    if (selected.has("claude")) {
      ...
      if (r.ok) wiredHosts.push("claude");
    }
    if (selected.has("claude-desktop")) {
      ...
      r.ok ? s.stop(pc.green("✓ ") + r.message) : s.stop(pc.yellow("! ") + r.message);
      if (r.ok) wiredHosts.push("claude-desktop");
    }
    if (selected.has("cursor")) {
      ...
      if (r.ok) wiredHosts.push("cursor");
    }
    if (selected.has("opencode")) {
      ...
      if (r.ok) wiredHosts.push("opencode");
    }
    for (const host of ["hermes", "openclaw", "grok"] as const) {
      if (!selected.has(host)) continue;
      const s = spinner();
      s.start(`Wiring ${host}…`);
      const r = wireHost(host, ctx)!;
      r.ok ? s.stop(pc.green("✓ ") + r.message) : s.stop(pc.yellow("! ") + r.message);
      if (r.ok) wiredHosts.push(host);
    }
...
function defaultPicks(
  hosts: ReturnType<typeof detectHosts>,
  scope: "user" | "project",
): HostId[] {
  const picks: HostId[] = [];
  if (hosts.claude) picks.push("claude");
  if (scope === "user") {
    if (hosts.claudeDesktop) picks.push("claude-desktop");
    if (hosts.cursor) picks.push("cursor");
    if (hosts.opencode) picks.push("opencode");
    if (hosts.codex) picks.push("codex");
    if (hosts.hermes) picks.push("hermes");
    if (hosts.openclaw) picks.push("openclaw");
    if (hosts.grok) picks.push("grok");
  }
  return picks;
}
