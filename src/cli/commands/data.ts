...
function resolveOutput(out: string | undefined, fallback: string): string {
  if (!out) return fallback;
  return isAbsolute(out) ? out : resolve(process.cwd(), out);
}

function fretworkHomePath(...parts: string[]): string {
...
export function runExport(opts: ExportOpts): void {
  ...
  if (opts.csv) {
    const dir = resolveOutput(
      opts.output,
      fretworkHomePath("exports", `csv-${new Date().toISOString().slice(0, 10)}`),
    );
    ...
    if (opts.json) return emitJson({ dir, files: Object.keys(files), bytes });
    emitOk(`Wrote ${Object.keys(files).length} CSV file(s) to ${dir} (${bytes} bytes).`);
    return;
  }
  const path = resolveOutput(
    opts.output,
    fretworkHomePath("exports", `fretwork-${new Date().toISOString().slice(0, 10)}.json`),
  );
  mkdirSync(dirname(path), { recursive: true });
  const payload = JSON.stringify(snapshot, null, 2);
  writeFileSync(path, payload, "utf8");
  if (opts.json) return emitJson({ path, bytes: payload.length });
...
export function runImport(file: string, opts: ImportOpts): void {
  ...
  if (mode === "replace" && !opts.force) {
    fail(
      `--mode replace will wipe ALL existing data. Re-run with --force to confirm.`,
    );
  }
  ...
  if (opts.json) return emitJson(summary);
  ...
  for (const err of summary.errors) process.stdout.write(`  ! ${err}\n`);
}
