...
export function runCrmNote(opts: AddOpts): void {
  if (!opts.client) fail("--client is required");
  if (!opts.body) fail("--body is required");
  const n = tryRun(
    () =>
      addCrmNote({
        client: opts.client,
  ...
  if (opts.json) return emitJson(n);
...
export function runCrmNotes(opts: ListNotesOpts): void {
  const rows = tryRun(
    () =>
      listCrmNotes({
        client: opts.client,
...
export function runCrmNoteGet(id: string, opts: { json?: boolean }): void {
  const n = tryRun(() => getCrmNote(id), opts.json);
  if (!n) fail(`not found: ${id}`);
  if (opts.json) return emitJson(n);
  process.stdout.write(JSON.stringify(n, null, 2) + "\n");
}

interface UpdateNoteOpts {
  client?: string;
  body?: string;
  date?: string;
  followup?: string;
...
export function runCrmNoteUpdate(id: string, opts: UpdateNoteOpts): void {
  ...
  if (opts.clearFollowup) patch.followupAt = null;
  else if (opts.followup !== undefined) patch.followupAt = opts.followup;
  const n = tryRun(() => updateCrmNote(id, patch), opts.json);
  if (opts.json) return emitJson(n);
  emitOk(`Updated CRM note ${n.id}.`);
}

export function runCrmNoteDelete(id: string, opts: { json?: boolean }): void {
  const n = tryRun(() => deleteCrmNote(id), opts.json);
  if (opts.json) return emitJson({ deleted: n });
  emitOk(`Deleted CRM note ${n.id}.`);
}

interface FollowupsOpts {
  staleness?: string;
  dueBy?: string;
  json?: boolean;
}

export function runCrmFollowups(opts: FollowupsOpts): void {
  ...
  const rows = tryRun(
    () => listFollowups({ stalenessDays, dueBy: opts.dueBy }),
    opts.json,
  );
  if (opts.json) return emitJson(rows);
  emitTable(
