...
export async function runInit(args: InitArgs): Promise<number> {
  ...
  const ask = async (prompt: string, fallback: string | number | null): Promise<string> => {
    if (!interactive) return fallback === null ? "" : String(fallback);
  ...
  if (!businessEmail) fail("Business email is required.");
  ...
  const paymentTerms =
    args.paymentTerms ?? (await ask("Payment instructions printed on invoices, e.g. bank details (optional)", before.paymentTerms ?? ""));
