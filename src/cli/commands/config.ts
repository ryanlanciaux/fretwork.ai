...
interface ShowOpts { json?: boolean }
export function runConfigShow(opts: ShowOpts): void {
  const c = getConfig();
...
export function runConfigSet(opts: SetOpts): void {
  const numberOpt = (v: string | undefined): number | undefined => {
    if (v === undefined) return undefined;
    const n = Number(v);
    if (!Number.isFinite(n)) fail(`Expected a number, got: ${v}`);
    return n;
  };
  const cfg = tryRun(
    () =>
  ...
  if (opts.json) return emitJson(cfg);
